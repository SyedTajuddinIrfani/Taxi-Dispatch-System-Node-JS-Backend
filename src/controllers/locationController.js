...
const create = async (req, res) => {
  try {
    ...
    req.body.location_type_id = req.body.location_type_id ? parseInt(req.body.location_type_id) : null;

    req.body.extra_charges = req.body.extra_charges ? parseFloat(req.body.extra_charges) : null;
    req.body.pickup_charges = req.body.pickup_charges ? parseFloat(req.body.pickup_charges) : null;
    req.body.dropoff_charges = req.body.dropoff_charges ? parseFloat(req.body.dropoff_charges) : null;
    req.body.latitude = req.body.latitude ? parseFloat(req.body.latitude) : null;
    req.body.longitude = req.body.longitude ? parseFloat(req.body.longitude) : null;

    // Convert "true"/"false" strings to boolean
    if (req.body.blacklist === 'true' || req.body.blacklist === true) req.body.blacklist = true;
    else if (req.body.blacklist === 'false' || req.body.blacklist === false) req.body.blacklist = false;
...
const remove = async (req, res) => {
  try {
    ...
    if (!deleted) return res.status(404).json({ status: false, message: 'Location not found' });
    res.json({ status: true, message:"Delete Location Successfully",location: deleted });
  } catch (err) {
    console.error(err);
    res.status(500).json({ status: false, message: 'Server error' });
  }
